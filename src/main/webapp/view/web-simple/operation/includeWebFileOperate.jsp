	...
	<div style="padding: 2px;">
		<c:if test="${USEROBJ!=null}">
			...
			<DOC:canWriteIsShow docId="${DOCE.doc.id}">
				...
				<c:if  test="${DOCE.files!=null  && fn:length(DOCE.files)==1 && (DOCE.files[0].type=='2'||DOCE.files[0].type=='1')  && (DOCE.files[0].exname=='.doc'|| DOCE.files[0].exname == '.docx' || DOCE.files[0].exname=='.xlsx'|| DOCE.files[0].exname == '.xls' || DOCE.files[0].exname == '.pdf' || DOCE.files[0].exname=='.ppt'|| DOCE.files[0].exname == '.pptx' )}">
					<button type="button" id="onlineEditButtonId"
					class="btn btn-default  btn-xs">
					<span class="glyphicon glyphicon-envelope"></span>&nbsp;在线编辑
					</button>
					</c:if>
			</DOC:canWriteIsShow>
...
<script type="text/javascript">
	$(function() {
		$('#onlineEditButtonId').bind(
				'click',
				function() {
					var tempwindow=window.open("");
					tempwindow.location=basePath
							+ 'pageoffice/openfile.do?fileId=${DOCE.files[0].id}';
				});
